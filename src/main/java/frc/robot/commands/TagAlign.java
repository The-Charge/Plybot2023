    ...
    public void execute() {
        ...
        if (result.hasTargets()) {
            ...
            PathPlannerTrajectory traj1 = PathPlanner.generatePath(
                ...
                new PathPoint(new Translation2d(Math.cos(yaw) + side * Constants.nodeSideDistanceMeters, Math.sin(yaw)), Rotation2d.fromDegrees(90)) // position (plus/minus side we aim for), heading
            );
            ...
            SmartDashboard.putNumber("X", Math.cos(yaw)); //X distance between Robot and Tag
            SmartDashboard.putNumber("Y", Math.sin(yaw)); //Y distance between Robot and Tag
